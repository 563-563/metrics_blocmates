# Holder Multiple — Cohort Snapshot

**Generated:** 2026-08-11T13:39:21.164Z
**As-of:** 2026-08-11
**Mode:** live

## Cohort summary

| Protocol | Phase | Adj MCap | Real Capture | HM | Band |
|---|---|---|---|---|---|
| Morpho (MORPHO) | `proposed` | $1.30B | $0/yr | **∞×** | no real capture |
| Aave (AAVE) | `current` | $1.34B | $0/yr | **∞×** | no real capture |
| Maple Finance (SYRUP) | `none` | $187.39M | $0/yr | **∞×** | no real capture |
| Hyperliquid (HYPE) | `current` | $26.99B | $89.06M/yr | **303.0×** | speculative |
| Uniswap (UNI) | `executing` | $2.40B | $62.76M/yr | **38.2×** | expensive |
| Sky (SKY) | `phase_1` | $1.24B | $0/yr | **∞×** | no real capture |
| Ethena (ENA) | `none` | $878.06M | $0/yr | **∞×** | no real capture |
| Meteora (MET) | `proposed` | $83.74M | $0/yr | **∞×** | no real capture |
| Raydium (RAY) | `executing` | $169.79M | $7.01M/yr | **24.2×** | fair value |
| Jito (JTO) | `paused` | $274.75M | $0/yr | **∞×** | no real capture |
| Aerodrome (AERO) | `executing` | $410.12M | $116.79M/yr | **3.5×** | exceptional |
| Sanctum (CLOUD) | `none` | $12.87M | $0/yr | **∞×** | no real capture |
| Kamino (KMNO) | `none` | $95.58M | $0/yr | **∞×** | no real capture |
| Ondo Finance (ONDO) | `proposed` | $1.65B | $0/yr | **∞×** | no real capture |
| Fluid (FLUID) | `executing` | $101.27M | $4.75M/yr | **21.3×** | fair value |
| Euler (EUL) | `executing` | $31.80M | $2.30M/yr | **13.8×** | strong |
| Collector Crypt (CARDS) | `executing` | $290.34M | $68.32M/yr | **4.2×** | exceptional |
| Drift (DRIFT) | `none` | $8.30M | $0/yr | **∞×** | no real capture |
| Dolomite (DOLO) | `executing` | $12.11M | $0/yr | **∞×** | no real capture |
| Pendle (PENDLE) | `executing` | $232.31M | $7.25M/yr | **32.0×** | fair value |
| Lido DAO (LDO) | `conditional` | $243.62M | $0/yr | **∞×** | no real capture |
| GMX (GMX) | `paused` | $67.52M | $0/yr | **∞×** | no real capture |
| dYdX (DYDX) | `executing` | $96.67M | $2.58M/yr | **37.5×** | expensive |
| Gains Network (GNS) | `executing` | $12.67M | $1.08M/yr | **11.7×** | strong |
| ether.fi (ETHFI) | `executing` | $373.74M | $5.10M/yr | **73.3×** | speculative |
| Curve Finance (CRV) | `executing` | $411.20M | $7.27M/yr | **56.6×** | speculative |
| Orca (ORCA) | `executing` | $63.96M | $587.53K/yr | **108.9×** | speculative |
| CoW Protocol (COW) | `executing` | $60.99M | $8.30M/yr | **7.3×** | exceptional |
| LayerZero (ZRO) | `executing` | $305.95M | $1.58M/yr | **193.7×** | speculative |
| Synthetix (SNX) | `unverified` | $120.24M | $0/yr | **∞×** | no real capture |
| Usual Money (USUAL) | `executing` | $16.36M | $11.96M/yr | **1.4×** | exceptional |
| pump.fun (PUMP) | `executing` | $1.06B | $208.95M/yr | **5.1×** | exceptional |
| Rollbit (RLB) | `executing` | $113.58M | $0/yr | **∞×** | no real capture |
| Jupiter Exchange (JUP) | `executing` | $598.25M | $22.48M/yr | **26.6×** | fair value |
| Marinade Finance (MNDE) | `executing` | $9.83M | $0/yr | **∞×** | no real capture |
| Lighter (LIT) | `current` | $1.15B | $24.09M/yr | **47.9×** | expensive |

## Per-protocol breakdown

### Morpho (MORPHO) — HM ∞× _(no real capture)_

Phase: `proposed` — Fee switch exists but not yet activated. Governance vote needed. Est. ~$20M/yr when live.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $1.98 | source: live |
| 2 | Current float market cap | $1.30B | 656,198,440 MORPHO × $1.98 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$1.30B** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to MORPHO (Category B) | $0 | fee-share (proposed): Fee switch exists but not yet activated. Governance vote needed. Est. ~$20M/yr when live. (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Aave (AAVE) — HM ∞× _(no real capture)_

Phase: `current` — ARFC $30M/yr buyback budget (cut from original $50M). AWW Framework commits 100% product revenue to DAO.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $88.89 | source: live |
| 2 | Current float market cap | $1.37B | 15,422,582 AAVE × $88.89 (circ source: live) |
| 3 | + 24mo unlocks | $0 | No team vesting; 99.9% circulating |
| 4 | + 24mo emissions | +$25.33M | Safety Module + Service Provider compensation (285K AAVE over 24mo) |
| 5 | − 24mo buybacks | −$60.00M | ARFC $30M/yr budget; seed value — overridden by onchain feed when present (verification: onchain_aggregate) |
| 6 | **Adjusted MCap** | **$1.34B** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | last 60d annualized — verification: onchain |
| 8 | Annual external cashflow yield to AAVE (Category B) | $0 | No Category B yield — Safety Module rewards are stkAAVE emissions (Category C, dilution rebate) (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

**Buyback rate lens:** recent 60d annualized = **$0/yr** (HM input). Lifetime annualized (332d, cumulative $22.33M) = **$24.55M/yr**. Recent rate is **-100.0%** below lifetime average.

[Sources: DefiLlama, TokenLogic dashboard, Aave governance forum]

---

### Maple Finance (SYRUP) — HM ∞× _(no real capture)_

Phase: `none` — No documented mechanism routing institutional lending revenue to SYRUP holders. syrupUSDC is a separate yield product.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.1506 | source: live |
| 2 | Current float market cap | $187.39M | 1,244,677,493 SYRUP × $0.150553 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$187.39M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to SYRUP (Category B) | $0 |  (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Hyperliquid (HYPE) — HM 303.0× _(speculative)_

Phase: `current` — Assistance Fund captures ~99% of revenue for HYPE buybacks. Team vesting cliff active.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $54.94 | source: live |
| 2 | Current float market cap | $16.41B | 298,649,468 HYPE × $54.94 (circ source: onchain_feed) |
| 3 | + 24mo unlocks | +$11.44B | Y1 119M + Y2 89.3M team unlocks (Tokenomist scheduled rate) |
| 4 | + 24mo emissions | +$373.59M | Staking emissions paid from Future Emissions allocation (Category C — added to dilution side) |
| 5 | − 24mo buybacks | −$1.24B | 30d revenue × 12 × 99% AF capture rate (seed value — overridden by onchain feed when present) (verification: onchain) |
| 6 | **Adjusted MCap** | **$26.99B** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $89.06M | last 60d annualized — verification: onchain |
| 8 | Annual external cashflow yield to HYPE (Category B) | $0 | No Category B — staking rewards are HYPE-denominated dilution rebate (Category C) (verification: governance_stated) |
| 9 | **Total Real Capture** | **$89.06M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **303.0×** | Line 6 ÷ Line 9 |

**Buyback rate lens:** recent 60d annualized = **$89.06M/yr** (HM input). Lifetime annualized (509d, cumulative $918.68M) = **$658.78M/yr**. Recent rate is **-86.5%** below lifetime average.

[Sources: DefiLlama, Hyperliquid Info API, Tokenomist, Hyperliquid docs]

---

### Uniswap (UNI) — HM 38.2× _(expensive)_

Phase: `executing` — 17% of LP fees → TokenJar → Firepit burn. Active since Dec 2025. DL revenue_1y = protocol's share of fees. 100% of that goes to Firepit.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $3.84 | source: live |
| 2 | Current float market cap | $2.40B | 624,316,424 UNI × $3.84 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback-burn (executing): 17% of LP fees → TokenJar → Firepit burn. Active since Dec 2025. DL revenue_1y = protocol's share of fees. 100% of that goes to Firepit. (verification: proxy) |
| 6 | **Adjusted MCap** | **$2.40B** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $62.76M | verification: proxy |
| 8 | Annual external cashflow yield to UNI (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$62.76M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **38.2×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Sky (SKY) — HM ∞× _(no real capture)_

Phase: `phase_1` — Phase 1 of TMF framework. SBE bypassed during ABC fill (~15 months base case from Apr 2026). 40% of net revenue → ABC, 40% → SKY stakers (as USDS), 20% → Security/Maintenance.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.0529 | source: live |
| 2 | Current float market cap | $1.24B | 23,411,391,348 SKY × $0.052852 (circ source: live) |
| 3 | + 24mo unlocks | $0 | 98.9% circulating; no remaining schedule |
| 4 | + 24mo emissions | $0 | Staking yield paid in USDS, not new SKY |
| 5 | − 24mo buybacks | $0 | SBE bypassed during Phase 1; verified on-chain — MCD_FLAP + MCD_SPLIT have zero activity 90d (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$1.24B** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to SKY (Category B) | $0 | Article: 40% × $180.73M net revenue paid in USDS to stkSKY (Cat B). On-chain (REWARDS_LSSKY_USDS = 0x38E4254b...) confirms USDS distribution mechanism — but inflows stopped 2025-11-03, 200+ days dormant. Either restructured post-Apr-2026 framework or paused. (verification: onchain_dormant) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama, Sky ChainLog (REWARDS_LSSKY_USDS resolved on-chain), Sky governance forum]

---

### Ethena (ENA) — HM ∞× _(no real capture)_

Phase: `none` — 99% of gross revenue passes to sUSDe holders by design. ENA captures only ~$2.6M of $212M fees (mint fees only). Core lesson: protocol value ≠ token value.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.0893 | source: live |
| 2 | Current float market cap | $878.06M | 9,828,125,000 ENA × $0.089342 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$878.06M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to ENA (Category B) | $0 |  (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Meteora (MET) — HM ∞× _(no real capture)_

Phase: `proposed` — Fee-sharing proposed but not yet live. MET staking with protocol fee-share in development.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.154 | source: live |
| 2 | Current float market cap | $83.74M | 543,713,307 MET × $0.154023 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$83.74M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to MET (Category B) | $0 | fee-share (proposed): Fee-sharing proposed but not yet live. MET staking with protocol fee-share in development. (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Raydium (RAY) — HM 24.2× _(fair value)_

Phase: `executing` — 12% of all trading fees → automatic RAY buyback & burn. 71M RAY burned to date. Note: uses fees_1y as base (not revenue_1y).

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.63 | source: live |
| 2 | Current float market cap | $169.79M | 269,513,825 RAY × $0.62999 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback-burn (executing): 12% of all trading fees → automatic RAY buyback & burn. 71M RAY burned to date. Note: uses fees_1y as base (not revenue_1y). (verification: proxy) |
| 6 | **Adjusted MCap** | **$169.79M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $7.01M | verification: proxy |
| 8 | Annual external cashflow yield to RAY (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$7.01M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **24.2×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Jito (JTO) — HM ∞× _(no real capture)_

Phase: `paused` — CSD sub-DAO committed to 100% of network revenue. $3.2M executed via TWAP as of Oct 2025. 4/6 multisig. JIP-24 routes all Block Engine + BAM fees to DAO.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.5401 | source: live |
| 2 | Current float market cap | $274.75M | 508,736,880 JTO × $0.540069 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (paused): CSD sub-DAO committed to 100% of network revenue. $3.2M executed via TWAP as of Oct 2025. 4/6 multisig. JIP-24 routes all Block Engine + BAM fees to DAO. (verification: onchain_dormant) |
| 6 | **Adjusted MCap** | **$274.75M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: onchain_dormant |
| 8 | Annual external cashflow yield to JTO (Category B) | $0 |  (verification: onchain_dormant) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Aerodrome (AERO) — HM 3.5× _(exceptional)_

Phase: `executing` — 100% of trading fees → veAERO lockers. No protocol treasury take. Algorithmic, structural. Revenue exceeded emissions Oct 2025. $295M+ cumulative distributed.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.4216 | source: live |
| 2 | Current float market cap | $410.12M | 972,794,617 AERO × $0.421588 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: proxy) |
| 6 | **Adjusted MCap** | **$410.12M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: proxy |
| 8 | Annual external cashflow yield to AERO (Category B) | $116.79M | fee-share-lockers (executing): 100% of trading fees → veAERO lockers. No protocol treasury take. Algorithmic, structural. Revenue exceeded emissions Oct 2025. $295M+ cumulative distributed. (verification: proxy) |
| 9 | **Total Real Capture** | **$116.79M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **3.5×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Sanctum (CLOUD) — HM ∞× _(no real capture)_

Phase: `none` — Zero protocol revenue flows to CLOUD. Protocol earns ~$6M/yr, retained by treasury. ASR was CLOUD emissions, not protocol revenue. Community discussions ongoing.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.0215 | source: live |
| 2 | Current float market cap | $12.87M | 599,013,117 CLOUD × $0.02148389 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$12.87M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to CLOUD (Category B) | $0 |  (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Kamino (KMNO) — HM ∞× _(no real capture)_

Phase: `none` — Protocol captures ~18% of fees (~$10M revenue) to treasury. No buyback, burn, or fee-share for KMNO token.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.0181 | source: live |
| 2 | Current float market cap | $95.58M | 5,276,027,713 KMNO × $0.01811545 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$95.58M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to KMNO (Category B) | $0 |  (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Ondo Finance (ONDO) — HM ∞× _(no real capture)_

Phase: `proposed` — Governance-only token today. Fee switch scheduled H2 2026. Ondo Chain mainnet may add gas/staking role. Zero confirmed accrual currently.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.3379 | source: live |
| 2 | Current float market cap | $1.65B | 4,869,330,647 ONDO × $0.337875 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$1.65B** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to ONDO (Category B) | $0 | fee-share (proposed): Governance-only token today. Fee switch scheduled H2 2026. Ondo Chain mainnet may add gas/staking role. Zero confirmed accrual currently. (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Fluid (FLUID) — HM 21.3× _(fair value)_

Phase: `executing` — 100% of mainnet revenue → FLUID buybacks. Approved and active since Oct 2025. Fully algorithmic, no discretion.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $1.21 | source: live |
| 2 | Current float market cap | $101.27M | 83,696,997 FLUID × $1.21 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (executing): 100% of mainnet revenue → FLUID buybacks. Approved and active since Oct 2025. Fully algorithmic, no discretion. (verification: proxy) |
| 6 | **Adjusted MCap** | **$101.27M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $4.75M | verification: proxy |
| 8 | Annual external cashflow yield to FLUID (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$4.75M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **21.3×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Euler (EUL) — HM 13.8× _(strong)_

Phase: `executing` — FeeFlow automated on-chain buyback mechanism. ~$1.82M annualized. ~1.5% supply repurchased annually.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $1.17 | source: live |
| 2 | Current float market cap | $31.80M | 27,182,818 EUL × $1.17 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (executing): FeeFlow automated on-chain buyback mechanism. ~$1.82M annualized. ~1.5% supply repurchased annually. (verification: proxy) |
| 6 | **Adjusted MCap** | **$31.80M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $2.30M | verification: proxy |
| 8 | Annual external cashflow yield to EUL (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$2.30M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **13.8×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Collector Crypt (CARDS) — HM 4.2× _(exceptional)_

Phase: `executing` — 85-90% of pack revenue → CARDS buyback & burn at market. Built into every pack purchase on Solana. Most aggressive mechanism in portfolio.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.1452 | source: live |
| 2 | Current float market cap | $290.34M | 1,999,705,628 CARDS × $0.145192 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback-burn (executing): 85-90% of pack revenue → CARDS buyback & burn at market. Built into every pack purchase on Solana. Most aggressive mechanism in portfolio. (verification: proxy) |
| 6 | **Adjusted MCap** | **$290.34M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $68.32M | verification: proxy |
| 8 | Annual external cashflow yield to CARDS (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$68.32M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **4.2×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Drift (DRIFT) — HM ∞× _(no real capture)_

Phase: `none` — $285M exploit April 2026, operations paused. DRIFT staking = fee discounts only, not yield. $1M buyback DIP-4 proposed but not confirmed executing. DIP-9 sends $1.5M/mo to Drift Labs.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.0116 | source: live |
| 2 | Current float market cap | $8.30M | 715,234,160 DRIFT × $0.0116015 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$8.30M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to DRIFT (Category B) | $0 |  (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Dolomite (DOLO) — HM ∞× _(no real capture)_

Phase: `executing` — Fee-share mechanism exists but entirely WLFI-partnership dependent. Flow to DOLO token holders unconfirmed. TVL impaired ($399M→$140M). accrual_pct=0 pending confirmation.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.0238 | source: live |
| 2 | Current float market cap | $12.11M | 509,723,226 DOLO × $0.02375157 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: proxy) |
| 6 | **Adjusted MCap** | **$12.11M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: proxy |
| 8 | Annual external cashflow yield to DOLO (Category B) | $0 | fee-share (executing): Fee-share mechanism exists but entirely WLFI-partnership dependent. Flow to DOLO token holders unconfirmed. TVL impaired ($399M→$140M). accrual_pct=0 pending confirmation. (verification: proxy) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Pendle (PENDLE) — HM 32.0× _(fair value)_

Phase: `executing` — 80% of swap fees + all YT yield → vePENDLE. Smart contract enforced, no governance discretion. sPENDLE provides liquid staking alternative.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $1.35 | source: live |
| 2 | Current float market cap | $232.31M | 172,080,412 PENDLE × $1.35 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: proxy) |
| 6 | **Adjusted MCap** | **$232.31M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: proxy |
| 8 | Annual external cashflow yield to PENDLE (Category B) | $7.25M | fee-share-lockers (executing): 80% of swap fees + all YT yield → vePENDLE. Smart contract enforced, no governance discretion. sPENDLE provides liquid staking alternative. (verification: proxy) |
| 9 | **Total Real Capture** | **$7.25M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **32.0×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Lido DAO (LDO) — HM ∞× _(no real capture)_

Phase: `conditional` — $20M discretionary buyback approved Apr 13 2026, not executing yet. NEST automated buybacks conditional on ETH >$3K. Zero accrual until first tranche executes.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.2913 | source: live |
| 2 | Current float market cap | $243.62M | 836,305,815 LDO × $0.291299 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (conditional): $20M discretionary buyback approved Apr 13 2026, not executing yet. NEST automated buybacks conditional on ETH >$3K. Zero accrual until first tranche executes. (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$243.62M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to LDO (Category B) | $0 |  (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### GMX (GMX) — HM ∞× _(no real capture)_

Phase: `paused` — 27% of V2 fees (30% V1) → algorithmic weekly Deposit-Buy-Withdraw cycle. Buybacks ARE executing on-chain (2M+ GMX repurchased since end-2024, verified in GMX dApp). PAUSED status = distribution suspended until GMX >= $90 (March 2026 DAO vot…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $6.46 | source: live |
| 2 | Current float market cap | $67.52M | 10,451,924 GMX × $6.46 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (paused): 27% of V2 fees (30% V1) → algorithmic weekly Deposit-Buy-Withdraw cycle. Buybacks ARE executing on-chain (2M+ GMX repurchased since end-2024, verified in GMX dApp). PAUSED status = distribution suspended until GMX >= $90 (March 2026 DAO vot… (verification: onchain_dormant) |
| 6 | **Adjusted MCap** | **$67.52M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: onchain_dormant |
| 8 | Annual external cashflow yield to GMX (Category B) | $0 |  (verification: onchain_dormant) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### dYdX (DYDX) — HM 37.5× _(expensive)_

Phase: `executing` — 75% of fees → TWAP buyback by Treasury SubDAO (on-chain, verifiable). 8.46M DYDX repurchased through Jan 2026. Stakers earn 15% of fees in USDC. WARNING: forward fee run rate $3.9M/yr (84% YoY collapse). Bought tokens are staked to validato…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.1139 | source: live |
| 2 | Current float market cap | $96.67M | 848,599,091 DYDX × $0.113912 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (executing): 75% of fees → TWAP buyback by Treasury SubDAO (on-chain, verifiable). 8.46M DYDX repurchased through Jan 2026. Stakers earn 15% of fees in USDC. WARNING: forward fee run rate $3.9M/yr (84% YoY collapse). Bought tokens are staked to validato… (verification: proxy) |
| 6 | **Adjusted MCap** | **$96.67M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $2.58M | verification: proxy |
| 8 | Annual external cashflow yield to DYDX (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$2.58M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **37.5×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Gains Network (GNS) — HM 11.7× _(strong)_

Phase: `executing` — MODEL CHANGED LATE 2024: SSS (real yield in DAI) discontinued. Now pure BB&B: algorithmic GNS buyback at 1hr TWAP +1% premium, then permanent burn. Stakers earn ZERO cash yield. DeFi Llama confirmed (2026-04-16): fees_1y $5.19M, revenue_1y …

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.5313 | source: live |
| 2 | Current float market cap | $12.67M | 23,839,587 GNS × $0.531299 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback-burn (executing): MODEL CHANGED LATE 2024: SSS (real yield in DAI) discontinued. Now pure BB&B: algorithmic GNS buyback at 1hr TWAP +1% premium, then permanent burn. Stakers earn ZERO cash yield. DeFi Llama confirmed (2026-04-16): fees_1y $5.19M, revenue_1y … (verification: proxy) |
| 6 | **Adjusted MCap** | **$12.67M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $1.08M | verification: proxy |
| 8 | Annual external cashflow yield to GNS (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$1.08M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **11.7×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### ether.fi (ETHFI) — HM 73.3× _(speculative)_

Phase: `executing` — 10% of protocol revenue allocated to ETHFI value accrual: 5% → buyback+burn (open market), 5% → sETHFI staker distributions (DAO Proposal #8). Additional $50M treasury buyback approved Nov 2025 (triggers at price < $3 — ACTIVE at $0.45). $6…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.3839 | source: live |
| 2 | Current float market cap | $373.74M | 973,468,000 ETHFI × $0.383929 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (executing): 10% of protocol revenue allocated to ETHFI value accrual: 5% → buyback+burn (open market), 5% → sETHFI staker distributions (DAO Proposal #8). Additional $50M treasury buyback approved Nov 2025 (triggers at price < $3 — ACTIVE at $0.45). $6… (verification: proxy) |
| 6 | **Adjusted MCap** | **$373.74M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $5.10M | verification: proxy |
| 8 | Annual external cashflow yield to ETHFI (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$5.10M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **73.3×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Curve Finance (CRV) — HM 56.6× _(speculative)_

Phase: `executing` — 50% of admin fee (not 50% of total trading fees) → veCRV weekly, swapped to crvUSD. Effective rate: 12.3% of gross user fees (DL holdersRevenue $4.8M / gross fees $39M). Plus crvUSD borrow fees ~$1.35M/yr fwd. Combined veCRV income ~$6M/yr.…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.2658 | source: live |
| 2 | Current float market cap | $411.20M | 1,546,807,897 CRV × $0.265841 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: proxy) |
| 6 | **Adjusted MCap** | **$411.20M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: proxy |
| 8 | Annual external cashflow yield to CRV (Category B) | $7.27M | fee-share-lockers (executing): 50% of admin fee (not 50% of total trading fees) → veCRV weekly, swapped to crvUSD. Effective rate: 12.3% of gross user fees (DL holdersRevenue $4.8M / gross fees $39M). Plus crvUSD borrow fees ~$1.35M/yr fwd. Combined veCRV income ~$6M/yr.… (verification: proxy) |
| 9 | **Total Real Capture** | **$7.27M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **56.6×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Orca (ORCA) — HM 108.9× _(speculative)_

Phase: `executing` — 40% of Whirlpool protocol fees → algorithmic xORCA buybacks (increased from 20% in Jan 2026). DL holdersRevenue confirms execution: $77.8K/30d → $933K/yr. 24-month program approved Aug 2025 with ~55K SOL treasury backstop + 30% ongoing fees…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $1.052 | source: live |
| 2 | Current float market cap | $63.96M | 60,798,778 ORCA × $1.052 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (executing): 40% of Whirlpool protocol fees → algorithmic xORCA buybacks (increased from 20% in Jan 2026). DL holdersRevenue confirms execution: $77.8K/30d → $933K/yr. 24-month program approved Aug 2025 with ~55K SOL treasury backstop + 30% ongoing fees… (verification: proxy) |
| 6 | **Adjusted MCap** | **$63.96M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $587.53K | verification: proxy |
| 8 | Annual external cashflow yield to ORCA (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$587.53K/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **108.9×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### CoW Protocol (COW) — HM 7.3× _(exceptional)_

Phase: `executing` — Net-negative emissions buyback: CIP mandates buybacks ≥ 1.2× weekly token emissions, making supply deflationary. ~80% of DAO revenue estimated to buybacks to maintain the ratio. DL holdersRevenue = $0 (DL does not categorize DAO treasury bu…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.106 | source: live |
| 2 | Current float market cap | $60.99M | 575,229,467 COW × $0.106024 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (executing): Net-negative emissions buyback: CIP mandates buybacks ≥ 1.2× weekly token emissions, making supply deflationary. ~80% of DAO revenue estimated to buybacks to maintain the ratio. DL holdersRevenue = $0 (DL does not categorize DAO treasury bu… (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$60.99M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $8.30M | verification: governance_stated |
| 8 | Annual external cashflow yield to COW (Category B) | $0 |  (verification: governance_stated) |
| 9 | **Total Real Capture** | **$8.30M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **7.3×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### LayerZero (ZRO) — HM 193.7× _(speculative)_

Phase: `executing` — Fee switch activated Dec 2025 (97% community vote) — 100% of LZ fees → ZRO burn (DL confirmed: holdersRevenue = fees = $249K/30d). Stargate V2 50% of bridge fees → ZRO buybacks (unverified by DL holdersRevenue). Total ZRO-attributable ~$4.0…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.8659 | source: live |
| 2 | Current float market cap | $305.95M | 353,313,326 ZRO × $0.865942 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback-burn (executing): Fee switch activated Dec 2025 (97% community vote) — 100% of LZ fees → ZRO burn (DL confirmed: holdersRevenue = fees = $249K/30d). Stargate V2 50% of bridge fees → ZRO buybacks (unverified by DL holdersRevenue). Total ZRO-attributable ~$4.0… (verification: proxy) |
| 6 | **Adjusted MCap** | **$305.95M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $1.58M | verification: proxy |
| 8 | Annual external cashflow yield to ZRO (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$1.58M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **193.7×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Synthetix (SNX) — HM ∞× _(no real capture)_

Phase: `unverified` — Q1 2026 tokenomics reset (SIP-2043): SNX inflation ended, 100% of perps trading fees → SNX buybacks (50/50 split with sUSD stability until peg restored, then 100%). DL shows $0 fees/revenue all recent periods under all slugs (synthetix, syn…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.207 | source: live |
| 2 | Current float market cap | $120.24M | 580,980,590 SNX × $0.206956 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (unverified): Q1 2026 tokenomics reset (SIP-2043): SNX inflation ended, 100% of perps trading fees → SNX buybacks (50/50 split with sUSD stability until peg restored, then 100%). DL shows $0 fees/revenue all recent periods under all slugs (synthetix, syn… (verification: governance_stated) |
| 6 | **Adjusted MCap** | **$120.24M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: governance_stated |
| 8 | Annual external cashflow yield to SNX (Category B) | $0 |  (verification: governance_stated) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Usual Money (USUAL) — HM 1.4× _(exceptional)_

Phase: `executing` — Fee switch Jan 7 2025 — up to 100% of protocol revenue to USUALx stakers in USD0. DL holdersRevenue shows near-zero execution: $3.6K/30d (0.16% of $2.22M/30d revenue). TTM distribution rate was 8.4% at peak TVL. TVL collapsed -94% from $1.7…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.0088 | source: live |
| 2 | Current float market cap | $16.36M | 1,858,355,554 USUAL × $0.00880354 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 |  (verification: proxy) |
| 6 | **Adjusted MCap** | **$16.36M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: proxy |
| 8 | Annual external cashflow yield to USUAL (Category B) | $11.96M | fee-share (executing): Fee switch Jan 7 2025 — up to 100% of protocol revenue to USUALx stakers in USD0. DL holdersRevenue shows near-zero execution: $3.6K/30d (0.16% of $2.22M/30d revenue). TTM distribution rate was 8.4% at peak TVL. TVL collapsed -94% from $1.7… (verification: proxy) |
| 9 | **Total Real Capture** | **$11.96M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **1.4×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### pump.fun (PUMP) — HM 5.1× _(exceptional)_

Phase: `executing` — DL confirmed: holdersRevenue $29.1M/30d ≈ fees $22.1M/30d (near-100% of revenue to PUMP buybacks). Protocol revenue = gross fees (100% retention — no LP share). $264.8M/yr fwd (TTM $391.7M, -32%). 23.2% annual buyback yield on Mcap $1.14B. …

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.0027 | source: live |
| 2 | Current float market cap | $1.06B | 393,247,541,040 PUMP × $0.00269613 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (executing): DL confirmed: holdersRevenue $29.1M/30d ≈ fees $22.1M/30d (near-100% of revenue to PUMP buybacks). Protocol revenue = gross fees (100% retention — no LP share). $264.8M/yr fwd (TTM $391.7M, -32%). 23.2% annual buyback yield on Mcap $1.14B. … (verification: proxy) |
| 6 | **Adjusted MCap** | **$1.06B** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $208.95M | verification: proxy |
| 8 | Annual external cashflow yield to PUMP (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$208.95M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **5.1×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Rollbit (RLB) — HM ∞× _(no real capture)_

Phase: `executing` — Hourly buyback-and-burn: ~10% casino + ~20% sportsbook + ~30% trading fees (blended ~20%). 3.296B RLB burned (65.9% of max supply 5B → current total 1.704B). No DL entry (centralized casino). Config revenue estimate: $18-30M/month GGR → $43…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.0711 | source: live |
| 2 | Current float market cap | $113.58M | 1,598,162,102 RLB × $0.071067 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback-burn (executing): Hourly buyback-and-burn: ~10% casino + ~20% sportsbook + ~30% trading fees (blended ~20%). 3.296B RLB burned (65.9% of max supply 5B → current total 1.704B). No DL entry (centralized casino). Config revenue estimate: $18-30M/month GGR → $43… (verification: proxy) |
| 6 | **Adjusted MCap** | **$113.58M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: proxy |
| 8 | Annual external cashflow yield to RLB (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Jupiter Exchange (JUP) — HM 26.6× _(fair value)_

Phase: `executing` — DL confirmed executing: 30d holdersRevenue $2.44M → $29.3M/yr (50% of protocol revenue). New mechanism active from 2025-02-17 (revenue-funded, not treasury-funded). All-time buybacks $90.9M. Original $70M treasury program exhausted 2024; Fe…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.1802 | source: live |
| 2 | Current float market cap | $598.25M | 3,320,312,968 JUP × $0.180179 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (executing): DL confirmed executing: 30d holdersRevenue $2.44M → $29.3M/yr (50% of protocol revenue). New mechanism active from 2025-02-17 (revenue-funded, not treasury-funded). All-time buybacks $90.9M. Original $70M treasury program exhausted 2024; Fe… (verification: proxy) |
| 6 | **Adjusted MCap** | **$598.25M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $22.48M | verification: proxy |
| 8 | Annual external cashflow yield to JUP (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$22.48M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **26.6×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Marinade Finance (MNDE) — HM ∞× _(no real capture)_

Phase: `executing` — DL confirmed executing: holdersRevenue $171K/30d → $2.05M/yr (3× TTM $675K — buyback was paused Dec 2025, has since resumed at higher rate). 50% of protocol revenue to MNDE buybacks. Protocol revenue $4.11M/yr fwd (TTM $7.15M, -43%). TVL $6…

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $0.018 | source: live |
| 2 | Current float market cap | $9.83M | 546,399,977 MNDE × $0.01798577 (circ source: live) |
| 3 | + 24mo unlocks | $0 | no editorial schedule — Adj MCap reflects float only |
| 4 | + 24mo emissions | $0 |  |
| 5 | − 24mo buybacks | $0 | buyback (executing): DL confirmed executing: holdersRevenue $171K/30d → $2.05M/yr (3× TTM $675K — buyback was paused Dec 2025, has since resumed at higher rate). 50% of protocol revenue to MNDE buybacks. Protocol revenue $4.11M/yr fwd (TTM $7.15M, -43%). TVL $6… (verification: proxy) |
| 6 | **Adjusted MCap** | **$9.83M** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $0 | verification: proxy |
| 8 | Annual external cashflow yield to MNDE (Category B) | $0 |  (verification: proxy) |
| 9 | **Total Real Capture** | **$0/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **∞×** | Line 6 ÷ Line 9 |

[Sources: DefiLlama (revenue/holders revenue), CoinGecko (price/supply), data/config.json (mechanism/status)]

---

### Lighter (LIT) — HM 47.9× _(expensive)_

Phase: `current` — TGE float. 99.5% of post-LLP revenue used to algorithmically buy back LIT. Team/investor cliff Dec 22 2026.

| # | Metric | Value | Notes |
|---|---|---|---|
| 1 | Token price | $2.47 | source: live |
| 2 | Current float market cap | $617.50M | 250,000,000 LIT × $2.47 (circ source: live) |
| 3 | + 24mo unlocks | +$533.52M | 54M Y1 + 162M Y2; cliff Dec 22 2026 unlocks 13.5M/mo across 36mo linear |
| 4 | + 24mo emissions | +$55.01M | Staking emissions over 24mo (Category C) |
| 5 | − 24mo buybacks | −$51.06M | 99.5% × post-LLP revenue. Proxy via DL holdersRevenue ($25.6M/yr, matches stated rate within 0.3%) until Lighter API key unlocks direct trade-level verification. (verification: proxy) |
| 6 | **Adjusted MCap** | **$1.15B** | Lines 2 + 3 + 4 − 5 |
| 7 | Annual buyback (Category A) | $24.09M | last 60d annualized — verification: onchain |
| 8 | Annual external cashflow yield to LIT (Category B) | $0 | No Category B — staking yield is LIT-denominated (Category C) (verification: governance_stated) |
| 9 | **Total Real Capture** | **$24.09M/yr** | Lines 7 + 8 |
| 10 | **Holder Multiple (HM)** | **47.9×** | Line 6 ÷ Line 9 |

**Buyback rate lens:** recent 60d annualized = **$24.09M/yr** (HM input). Lifetime annualized (218d, cumulative $24.21M) = **$40.53M/yr**. Recent rate is **-40.6%** below lifetime average.

[Sources: DefiLlama, Lighter docs]

## Methodology

- `Adjusted MCap = float_mcap + 24mo_unlocks + 24mo_emissions − 24mo_buybacks`
- `Total Real Capture = annual_buyback_usd + annual_holder_yield_usd` (Categories A + B; Category C token-denominated emissions are excluded)
- `HM = Adjusted MCap / Total Real Capture`

Bands per CLAUDE.md HM Interpretation Bands:

- **<10×** exceptional · **10–20×** strong · **20–35×** fair value · **35–50×** expensive · **>50×** speculative

Inputs sourced from `data/latest.json` (live DL+CG) for price/supply and from `data/hm/config.json` (editorial seed) for unlock schedules, buyback rates, and Category B yields. Buyback execution flagged with `verification` per the Buyback Quality Framework. v1 is lenient — `governance_stated` and `proxy` rows are accepted; `onchain` upgrade happens protocol-by-protocol.
