...
| Token alignment | **derived**: matched-window capture ÷ clean earnings, capped at 1. Capture resolution: on-chain feed trailing sum (`scripts/tg/capture.js`) > DL holders-revenue over the same window > HM 60d-SMA (legacy fallback) | **bound** — recomputed each run | Windows MUST match the revenue basis — HM's 60d SMA ÷ 1y revenue misread HYPE at 46% vs ~88% reality. `alignment_basis` in each token file records window + source. Paused mechanisms and capture-without-mechanism auto-flag questions. |
